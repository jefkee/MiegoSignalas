...
class ModelTrainer:
    ...
    def train_with_cross_validation(self, X, y, n_folds=5):
        ...
        for fold, (train_idx, val_idx) in enumerate(kfold.split(X)):
            ...
            # Save fold model
            self.save_model(f'models/sleep_classifier_fold{fold + 1}.h5')
            
            # Plot learning curves
            self._plot_learning_curves(history, fold + 1)
            
            # Evaluate and plot metrics
            cm, report = self._evaluate_fold(self.model.model, X_val, y_val, fold + 1)
            confusion_matrices.append(cm)
            reports.append(report)
            
            # Plot ROC curves
            y_pred_proba = self.model.model.predict(X_val)
            self._plot_roc_curves(y_val, y_pred_proba, fold + 1)
        
        print("\nCross-validation results:")
        print(f"Mean accuracy: {np.mean(fold_scores):.4f} ± {np.std(fold_scores):.4f}")
        
        # Save final results summary
        self._save_results_summary(fold_scores, confusion_matrices, reports)
