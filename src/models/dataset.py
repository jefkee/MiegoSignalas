...
class SleepDataset:
    ...
    def _augment_segment(self, segment):
        ...
        if np.random.random() < 0.2:
            mask_size = np.random.randint(50, 150)
            start = np.random.randint(0, segment.shape[0] - mask_size)
            segment[start:start+mask_size, :] = 0
        ...
        if np.random.random() < 0.3:
            channel = np.random.randint(0, segment.shape[1])
            segment[:, channel] = 0
