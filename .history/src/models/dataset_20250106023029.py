    ...
    def load_training_data(self):
        ...
        for psg_file in tqdm(psg_files, desc="Loading EDF files"):
            try:
                    ...
                    try:
                        ...
                        if end_idx <= data.shape[1]:
                            ...
                            try:
                                ...
                                file_features.append(segment)
                                file_labels.append(sleep_stages[i])
                                
                                # Add augmented version if enabled
                                if self.use_augmentation:
                                    aug_segment = self._augment_segment(segment.copy())
                                    if aug_segment.shape == EXPECTED_SHAPE:
                                        file_features.append(aug_segment)
                                        file_labels.append(sleep_stages[i])
                                    else:
                                        print(f"Warning: Invalid augmented segment shape: {aug_segment.shape}")
                                        
                            except Exception as e:
                                print(f"Error preprocessing segment {i}: {str(e)}")
                                continue
                ...
                # Convert file segments to numpy array to verify shapes
                if file_features:
                    try:
                        file_features = np.array(file_features)
                        file_labels = np.array(file_labels)
                        print(f"File segments shape: {file_features.shape}")
                        features.extend(file_features)
                        labels.extend(file_labels)
                    except Exception as e:
                        print(f"Error converting file segments to array: {str(e)}")
        ...
        try:
            ...
            raise ValueError(f"Error converting features to array: {str(e)}. Feature shapes may be inconsistent.")
