...
class SleepDataset:
    ...
    def load_training_data(self):
                ...
                if len(sleep_stages) > 0:
                    ...
                    for i in range(n_segments):
                        ...
                        if end_idx <= data.shape[1]:  # Only if we have enough data
                            ...
                            print(f"\nSegment {i} initial shape: {segment.shape}")
                            ...
                            if segment.shape[0] < len(required_channels):
                                ...
                                print(f"Segment {i} shape after padding: {segment.shape}")
                            
                            segment = segment.T  # Shape: (3000, n_channels)
                            print(f"Segment {i} shape after transpose: {segment.shape}")
                            
                            # Preprocess
                            try:
                                segment = self._preprocess_segment(segment)
                                print(f"Segment {i} shape after preprocessing: {segment.shape}")
                                ...
                                if self.use_augmentation:
                                    ...
                                    print(f"Augmented segment {i} shape: {aug_segment.shape}")
        ...
        if len(unique_shapes) > 1:
            ...
            raise ValueError(f"Inconsistent segment shapes found: {unique_shapes}")
    ...
    def _preprocess_segment(self, segment):
        ...
        try:
            # Ensure segment is exactly 3000 samples
            if segment.shape[0] != 3000:
                if segment.shape[0] > 3000:
                    # If longer, take the middle 3000 samples
                    start = (segment.shape[0] - 3000) // 2
                    segment = segment[start:start+3000, :]
                else:
                    # If shorter, pad with zeros to reach 3000
                    pad_size = 3000 - segment.shape[0]
                    pad_before = pad_size // 2
                    pad_after = pad_size - pad_before
                    segment = np.pad(segment, ((pad_before, pad_after), (0, 0)), 
                                   mode='constant', constant_values=0)
    ...
    def _augment_segment(self, segment):
        ...
        # Ensure input is exactly 3000 samples
        if segment.shape[0] != 3000:
            raise ValueError(f"Expected 3000 samples, got {segment.shape[0]}")
        ...
        if np.random.random() < 0.3:
            ...
            if new_len > 3000:
                # Take middle section
                start = (new_len - 3000) // 2
                segment = segment[start:start+3000]
            else:
                # Pad to 3000
                pad_size = 3000 - new_len
                pad_before = pad_size // 2
                pad_after = pad_size - pad_before
                segment = np.pad(segment, ((pad_before, pad_after), (0, 0)), 
                               mode='constant', constant_values=0)
